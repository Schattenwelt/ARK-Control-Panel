...
cat > "$SUDO_FILE" <<'SUDO'
ark ALL=(root) NOPASSWD: /usr/bin/systemctl enable --now ark.service, /usr/bin/systemctl disable --now ark.service, /usr/bin/systemctl enable ark.service, /usr/bin/systemctl disable ark.service, /usr/bin/systemctl restart ark.service, /usr/bin/systemctl start ark-update.service
SUDO
chmod 440 "$SUDO_FILE"
